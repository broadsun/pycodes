...
file_in = r"result3.txt"
file_out = r"result3.xls"
...
def sortData(file_in):
	...
	for line in fin:
		...
		line_num += 1
	all_data.sort(key=lambda l:(l[2],l[1]))
	new_data = []
	...
	while all_data_index < all_data_len-1:
		...
		if all_data[all_data_index][0] == '756' and all_data[all_data_index+1][0] == '759':
			tmp.append(all_data[all_data_index][1])
			tmp.append(map_cates.get(all_data[all_data_index][2]))
			tmp.append(all_data[all_data_index][3].strip())
			tmp.append(all_data[all_data_index+1][3].strip())
			tmp.append(round(float(tmp[3])/float(tmp[2]), 2)*100)
			new_data.append(tmp)
			all_data_index += 2
		elif all_data[all_data_index][0] == '756' and all_data[all_data_index+1][0] == '756':
			all_data_index += 1

	excel3 = xlwt.Workbook(encoding='utf-8', style_compression=0)
	sheet = excel3.add_sheet("sheet1", cell_overwrite_ok=True)
	sheet.write(0,0,'年龄')
	sheet.write(0,1,'领域')
	sheet.write(0,2,'曝光数')
	sheet.write(0,3,'点击数')
	sheet.write(0,4,'点击曝光百分比')
	data_line_num = 1
